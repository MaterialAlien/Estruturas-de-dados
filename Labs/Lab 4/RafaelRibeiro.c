...
{
    ptLSE *aux,*aux2=NULL; //ponteiros auxiliares para guardar uma posição no iteração da LSD
    ...
    if (aux2 == NULL) // Lista vazia, primeiro termo a ser inserido na lista
        ...
        {
            novo->ant=aux2;
            aux2->prox=novo; // Se o termo digitado não for encontrado na lista, então ele é adicionado no fim da lista
        }
